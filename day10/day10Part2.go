...
func main() {
	...
	cleanGrid := make([][]rune, len(grid))
	for i, row := range grid {
		cleanGrid[i] = make([]rune, len(row))
		for j := range row {
			cleanGrid[i][j] = '.'
		}
	}
	...
	currentY := startY
	for !backAtStart {
		cleanGrid[currentY][currentX] = grid[currentY][currentX]
	...
	fmt.Println("Clean grid:")
	for _, row := range cleanGrid {
		fmt.Println(string(row))
	}

	// use the similiar algorithm as in efalg to find points in a polygon
	enclosedTiles := 0
	for y, row := range cleanGrid {
		isInside := false
		for x, char := range row {
			if char == '.' {
				if isInside {
			...
			// any north pipe will change the inside/outside state
	...
	fmt.Println("Clean grid with enclosed tiles:")
	for _, row := range cleanGrid {
		fmt.Println(string(row))
	}
